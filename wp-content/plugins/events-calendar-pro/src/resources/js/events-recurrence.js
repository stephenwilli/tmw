...
( function( $, my ) {
	...
	my.add_recurrence = function( data ) {
		...
		$rule.find( '[name*="recurrence[rules][]"],[id*="recurrence_rule_--"],[data-depends*="#recurrence_rule_--"]' ).each( function() {
			var $field = $( this );
		...
		if ( ! data ) {
			...
			var default_day_of_week = moment( start_date, this.date_format ).isoWeekday();
			$rule.find( '[data-field="custom-week-day"][value="' + default_day_of_week + '"]' ).prop( 'checked', true );
		}
		...
		this.setup_yearly_checkboxes( $rule );

		this.init_dropdowns();

		if ( is_new ) {
			// pop open the picker to save everyone an extra click
			$rule.find( '[data-field="type"]' ).select2( 'open' );
		}
	 ...
	 */
	my.setup_yearly_checkboxes = function( $rule ) {
		...
		var $checkboxes = $rule.find( '[data-field="custom-year-month"]' );

		// don't override previous selections
		if ( $checkboxes.filter( ':checked' ).length ) {
			return;
		}

		$checkboxes.filter( '[value="' + start_month + '"]' ).prop( 'checked', true );
	};
	...
	my.add_exclusion = function( data ) {
		...
		$rule.find( '[name*="recurrence[exclusions][]"],[id*="exclusion_rule_--"],[data-depends*="#exclusion_rule_--"]' ).each( function() {
			var $field = $( this );
	...
	my.update_rule_recurrence_text = function( $rule ) {
		...
		if ( 'weekly' === type ) {
			...
			var month_number = $rule.find( '[data-field="custom-year-month-number"] option:selected' ).val();
			var month_day = $rule.find( '[data-field="custom-year-month-day"] option:selected' ).val();
	...
	my.event.recurrence_type_changed = function() {
		...
		my.setup_yearly_checkboxes( $rule );
